...
//        return "rbtm baseline";
    ...
    public static function RP_test_wpdb() {
        global $wpdb;
        $result = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'posts LIMIT 10');
        $frag_php = '<div class="test-wpdb">';
//        $frag_php .= '<pre>'.var_dump($result).'</pre>'; // insert code;
        $frag_php .= '</div>';
        return $frag_php;
    }
...
//  Lynda - WordPress - Plugin Development 120618 // 09-Plugin activation and deactivation
...
//  Lynda - WordPress - Plugin Development 110718 // 09-Plugin activation and deactivation
register_activation_hook( __FILE__, function(){
    if (!current_user_can( 'activate_plugins' )) return;
    add_option('RBTM_post_per_page', 10);
    add_option('RBTM_show_welcome_page', true);
    include_once dirname(__FILE__).'/includes/activation.php';    
});

register_deactivation_hook(__FILE__, function(){
    if (!current_user_can( 'activate_plugins' )) return;
    delete_option('RBTM_post_per_page');
    delete_option('RBTM_show_welcome_page');
    flush_rewrite_rules();
//    wp_die('Raymacz says - Plugin deactivated!!!');
});
